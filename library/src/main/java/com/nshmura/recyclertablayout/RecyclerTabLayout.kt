...
class RecyclerTabLayout @JvmOverloads constructor(
  ...
  fun setUpWithAdapter(adapter: Adapter<*>) {
    ...
    this.viewPager = adapter.viewPager.also { pager ->
      pager.addOnPageChangeListener(ViewPagerOnPageChangeListener(this))
      setAdapter(adapter)
  ...
  abstract class Adapter<T : ViewHolder?>(val viewPager: ViewPager) : RecyclerView.Adapter<T>() {
  ...
  class DefaultAdapter(viewPager: ViewPager) : Adapter<DefaultAdapter.ViewHolder>(viewPager) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
      val title = viewPager.adapter?.getPageTitle(position) ?: return
